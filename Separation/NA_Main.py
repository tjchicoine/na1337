...
class Main_Window:
    def __init__(self,master):
        ...
        self.s = ttk.Separator(p1,orient='horizontal')
        self.s.grid(columnspan=50,sticky=E+W,padx=10,pady=10)
        self.row1text_message = Message(p1,text = 'Row 1 Prefix',aspect=300,justify = CENTER).grid(row=10,sticky='W')
        self.row1text = Entry(p1)
        self.row1text.grid(row=10,column=1)
        self.row1text.config(state='disabled')
        self.row2text_message = Message(p1,text = 'Row 2 Prefix',aspect=200,justify = CENTER).grid(row=11,sticky='W')
        self.row2text = Entry(p1)
        self.row2text.grid(row=11,column=1)
        self.row2text.config(state='disabled')

        #---------------------Sequential Number -------------------------------#
        self.sequence1a_message = Message(p1,text = 'Row 1: Lowest:',aspect=900).grid(row=12,sticky='W')
        self.sequence1a = Entry(p1)
        self.sequence1a.grid(row=12,column=1)
        self.sequence1b_m = Message(p1,text = 'Row 1: Highest:',aspect = 900).grid(row = 13, sticky = 'w')
        self.sequence1b = Entry(p1)
        self.sequence1b.grid(row=13,column=1)
        self.sequence2_message = Message(p1,text = 'Row 2: Lowest:',aspect=800).grid(row=14,sticky='W')
        self.sequence2a = Entry(p1)
        self.sequence2a.grid(row=14,column=1)
        self.sequence2b_m = Message(p1,text = 'Row 2: Highest:', aspect=800).grid(row = 15,sticky = 'W')
        self.sequence2a.config(state='disabled')
        self.sequence2b = Entry(p1)
        self.sequence2b.grid(row=15,column=1)
